...
public StyledText(Composite parent, int style) {
	...
	addListener(SWT.ZoomChanged, event -> {
		if (event instanceof ZoomChangedEvent zoomChangedEvent) {
			handleStyledTextDPIChange(zoomChangedEvent);
		}
	});
}
/**
...
private void handleStyledTextDPIChange(ZoomChangedEvent event) {
	...
	caretSet.stream().filter(Objects::nonNull).forEach(caretToRefresh -> {
		ZoomChangedEvent caretEvent = event.createFor(caretToRefresh);
		((Caret) caretToRefresh).notifyListeners(SWT.ZoomChanged, caretEvent);
